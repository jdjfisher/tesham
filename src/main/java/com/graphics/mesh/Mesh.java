...
public class Mesh {
    ...
    public Mesh(MultiIndexMeshData meshData){
        this(meshData.toSingleIndexMeshData());
    }

    public Mesh(SingleIndexMeshData meshData){
        boolean needsNormals = false;
        boolean hasTextureCoords = true;
        boolean needsTangents = false;

        for (Vertex vertex : meshData.getVertices()) {
        ...
        glVertexAttribPointer(1, Vector3f.SIZE, GL_FLOAT, false, Vertex.SIZE * FLOAT_SIZE, 3 * FLOAT_SIZE);
        // texture coords
        glEnableVertexAttribArray(2);
        glVertexAttribPointer(2, Vector2f.SIZE, GL_FLOAT, false, Vertex.SIZE * FLOAT_SIZE, 6 * FLOAT_SIZE);
        // tangents
        glEnableVertexAttribArray(3);
        glVertexAttribPointer(3, Vector3f.SIZE, GL_FLOAT, false, Vertex.SIZE * FLOAT_SIZE, 8 * FLOAT_SIZE);
        // bitangents
        glEnableVertexAttribArray(4);
        glVertexAttribPointer(4, Vector3f.SIZE, GL_FLOAT, false, Vertex.SIZE * FLOAT_SIZE, 11 * FLOAT_SIZE);
    ...
    private FloatBuffer verticesToFloatBuffer(ArrayList<Vertex> vertices){
        final int floatCount = vertices.size() * Vertex.SIZE;
    ...
    private static void calcNormalsFlat(ArrayList<Vertex> vertices, ArrayList<FaceSI> faces){
        ArrayList<Vertex> newVerts = new ArrayList<>();
        ArrayList<FaceSI> newFaces = new ArrayList<>();

        int j = 0;

        for(FaceSI face : faces){
            Vertex[] faceVertices = face.getVertices(vertices);
            ...
            for(Vertex vertex : faceVertices){
                newVerts.add(new Vertex(vertex.getPosition(), faceNormal, vertex.getTextureCoord(), vertex.getTangent(), vertex.getBitangent()));
            }
    ...
    private static void calcNormalsSmooth(ArrayList<Vertex> vertices, ArrayList<FaceSI> faces){ // https://www.youtube.com/watch?v=PMgjVJogIbc
    ...
    private static void calcNormals(SingleIndexMeshData meshData){
        ArrayList<Vertex> vertices = meshData.getVertices();
        ...
        for(FaceSI face: faces){
            ...
            for(Vertex vertex : faceVertices){
                ...
                newVertices.add(new Vertex(vertex.getPosition(), finalNormal, vertex.getTextureCoord(), vertex.getTangent(), vertex.getBitangent()));
            }
    ...
    private static float calcBoundingSphereRadius(ArrayList<Vertex> vertices){
